...
# All cluster coordinates come from the environment (see .env.example); this
# file intentionally hardcodes no hosts, usernames or home directories.
_hydra_project_dir = os.environ.get('HYDRA_PROJECT_DIR', '')
hydra_base_dir = os.path.dirname(_hydra_project_dir) if _hydra_project_dir else ''
hpc_base_dir = os.environ.get('HPC_BASE_DIR', '')
known_hosts_path = os.environ.get(
    'KNOWN_HOSTS', os.path.expanduser('~/.ssh/known_hosts')
)
user_name = os.environ.get('HYDRA_SSH_USER', '')

CLUSTER_NAMES = ['hydra', 'hpc']
CLUSTER_DOMAINS = dict(
    hydra=os.environ.get('HYDRA_HOST', ''),
    hpc=os.environ.get('HPC_HOST', ''),
)
CLUSTER_BASE_DIRS = dict(hydra=hydra_base_dir, hpc=hpc_base_dir)


def _require(value: str, variable_name: str) -> str:
    if not value:
        raise SystemExit(f'Missing {variable_name}. Set it in your environment or .env.')
    return value
...
def copy_file_to_cluster(cluster_name: str, source_path: str, target_path: str) -> None:
    user = _require(user_name, 'HYDRA_SSH_USER')
    command = ['scp', source_path, f'{user}@{cluster_name}:{target_path}']
...
def get_ssh_connection_to_cluster(cluster_name: str) -> SSHClient:
    host = _require(CLUSTER_DOMAINS[cluster_name], f'{cluster_name.upper()}_HOST')
    client = SSHClient()
    client.load_host_keys(filename=known_hosts_path)
    client.connect(host, username=_require(user_name, 'HYDRA_SSH_USER'))
    return client
...
def main(cluster_name: str) -> None:
    ...
    base_dir = _require(
        ...
        'HYDRA_PROJECT_DIR' if cluster_name == 'hydra' else 'HPC_BASE_DIR',
    )
    target_path_compressed_code = join(base_dir, filename_compressed_code)
